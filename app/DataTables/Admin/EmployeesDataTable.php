...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('role', function ($row) use ($roles) {
                ...
                if ($row->id != 1) {
                    $btn = (($row->roleId == 1) ? 'btn-danger' : 'btn-info');
                    $roleName = ($row->current_role <= 3) ? __('app.' .$currentRoleName) : $currentRoleName;
    ...
    public function query(User $model)
    {
        ...
        // if ($request->inactive != 'all') {
        //     $users = $users->where('status', 'active');
        // }else{
        //    // $users = $users->where('status', $request->inactive);
        // }
