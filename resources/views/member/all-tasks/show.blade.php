...
<div class="r-panel-body p-t-0">
    ...
    <div class="row">
        <div class="col-xs-12 col-md-9 p-t-20 b-r h-scroll">
            ...
            <div class="col-xs-12">
                <a class="btn btn-default btn-sm m-b-10 btn-rounded btn-outline pull-right m-l-5" href="{{ route('member.all-tasks.download-task', $task->id) }}"> <span><i class="fa fa-file-pdf-o"></i></span> </a>
...
var myDropzone;
    ...
    $('body').on('click', '.file-edit', function () {
        ...
        var url = "{{ route('member.task-files.updates',':id') }}";
    ...
    $('#dounload-zip').click(function () {
        var files = [],
            ...
            url = "{{ route('member.task-files.alldownloadzip',':id') }}",
            token = "{{ csrf_token() }}";
            url = url.replace(':id', id);
