...
<script>
    ...
    $('#allTasks-table').on('click', '#changedate', function () {
        var thisdate = $(this).parent().find('#changedateinput'),
            thisinput = $('input', thisdate),
            thisbutton = $('#update-date', thisdate);

        thisdate.toggle('slow');
        ...
        $(thisbutton).click(function (e) { 
            var url = "{{route('admin.tasks.changeDate')}}";
            var token = "{{ csrf_token() }}";
            var id =  $(this).data('task-id');
            var date =  $(thisinput).val();

            $.easyAjax({
                url: url,
                type: "POST",
                data: {'_token': token, taskId: id, date: date},
                success: function (data) {
                    if (data.status == "success") {
                        window.LaravelDataTables["allTasks-table"].draw();
                    }
